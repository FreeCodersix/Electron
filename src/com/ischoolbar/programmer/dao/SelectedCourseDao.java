...
public class SelectedCourseDao extends BaseDao {
	public List<SelectedCourse> getSelectedCourseList(SelectedCourse selectedCourse,Page page){
		...
		if(selectedCourse.getStatus() != "" && selectedCourse.getStatus() != null){
			sql += " and status = '"+selectedCourse.getStatus()+"'";
		}
		...
		try {
			while(resultSet.next()){
				...
				cl.setStatus(resultSet.getString("status"));
				ret.add(cl);
			}
	...
	public boolean addSelectedCourse(SelectedCourse selectedCourse){
		String sql = "insert into s_selected_course values(null,"+selectedCourse.getStudentId()+","+selectedCourse.getCourseId()+",'"+selectedCourse.getStatus()+"')";
		return update(sql);
	}
	 ...
	 */
	public void updateSelectedCourse(SelectedCourse selectedCourse){
		String sql = "UPDATE s_selected_course s SET s.`status`='"+selectedCourse.getStatus()+"'" +
				" WHERE s.id="+selectedCourse.getId();
		update(sql);
	}
	/**
	 * 删除所选课程
	...
	public SelectedCourse getSelectedCourse(int id){
		...
		try {
			if(query.next()){
				...
				ret.setStatus(query.getString("status"));
				return ret;
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
