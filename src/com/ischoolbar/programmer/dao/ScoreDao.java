...
public class ScoreDao extends BaseDao {
	...
	public List<Score> getScoreList(Score score, Page page) {
		...
		String sql = "SELECT ss.*,st.sn,st.clazz_id classId,sc.`name` className FROM s_score ss LEFT JOIN s_student st ON ss.student_id=st.id LEFT JOIN s_clazz sc ON st.clazz_id = sc.id where 1=1 ";
		if(score.getStudentId() != 0){
			sql += " and ss.student_id = " + score.getStudentId();
		}
		if(score.getCourseId() != 0){
			sql += " and ss.course_id = " + score.getCourseId();
		}
		if(score.getClassId() != 0){
			sql += " and st.clazz_id = " + score.getClassId();
		}
		...
		try {
			while(resultSet.next()){
				...
				s.setClassName(resultSet.getString("className"));
				ret.add(s);
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
	...
	public List<Map<String, Object>> getScoreList(Score score) {
		...
		String sql = "SELECT\n" +
				...
				"\tst.sn,\n" +
				"\tsz.`name` AS className,\n" +
				"\tsc.NAME AS courseName \n" +
				"FROM\n" +
				"\ts_score se\n" +
				"\tLEFT JOIN s_student st ON se.student_id = st.id\n" +
				"\tLEFT JOIN s_clazz sz ON st.clazz_id = sz.id\n" +
				"\tLEFT JOIN s_course sc ON se.course_id = sc.id where 1=1 ";
		if(score.getStudentId() != 0){
			sql += " and se.student_id = " + score.getStudentId();
		}
		if(score.getCourseId() != 0){
			sql += " and se.course_id = " + score.getCourseId();
		}
		if(score.getClassId() != 0){
			sql += " and st.clazz_id = " + score.getClassId();
		}
		...
		try {
			while(resultSet.next()){
				...
				s.put("sn",resultSet.getString("sn"));
				s.put("className",resultSet.getString("className"));
				ret.add(s);
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
	...
	public int getScoreListTotal(Score score) {
		// TODO Auto-generated method stub
		int total = 0;
		String sql = "SELECT COUNT(*) as total FROM s_score ss LEFT JOIN s_student st ON ss.student_id=st.id LEFT JOIN s_clazz sc ON st.clazz_id = sc.id where 1=1 ";
		if(score.getStudentId() != 0){
			sql += " and ss.student_id = " + score.getStudentId();
		}
		if(score.getCourseId() != 0){
