		...
		$(function() {
			...
			$('#dataList').datagrid({
				...
				columns: [[
					...
					{field:'studentID',title:'学生',width:100,
					...
					{field:'studentNo',title:'学号',width:150,
						formatter: function(value,row,index){
							if (row.studentId){
								...
								for(var i=0;i<studentList.length;i++ ){
									//console.log(clazzList[i]);
									if(row.studentId == studentList[i].id)return row.sn;
								}
								return row.studentId;
							} else {
								return 'not found';
					...
					{field:'className',title:'班级',width:150,
						formatter: function(value,row,index){
							if (row.studentId){
								...
								for(var i=0;i<studentList.length;i++ ){
									//console.log(clazzList[i]);
									if(row.studentId == studentList[i].id)return row.className;
								}
								return row.studentId;
							} else {
								return 'not found';
				...
				onLoadSuccess:function () {
					...
					if(3 == <%=Integer.parseInt(session.getAttribute("userType").toString())%>) {
							...
							$('#dataList').datagrid('load', {
								...
								className: <%=session.getAttribute("classID")%>
							});
						flag = false
						}
					}
			...
			function preLoadClazz(){
				$("#studentList").combobox({
					...
					onChange: function(newValue, oldValue){
						//加载班级下的学生
						// $('#dataList').datagrid("options").queryParams = {className: newValue};
			...
			$("#delete").click(function(){
				...
				if(selectRows.length != 1){
					...
					if(selectRows[0].status != 0){
						$.messager.alert("消息提醒", "该记录已审核，如需删除请联系管理员!", "warning");
					}else{
			...
			//下拉框通用属性
			$("#add_studentList, #edit_studentList,#studentList,#classList,#statusList").combobox({
				width: "200",
				height: "30",
				valueField: "id",
				textField: "name",
			...
			$("#search-btn").click(function(){
				...
				$('#dataList').datagrid('load',{
					...
					className: $("#classList").combobox('getValue') == '' ? 0 : $("#classList").combobox('getValue')
				});
...
<body>
	...
	<div id="toolbar">
		...
		<div style="margin-top: 3px;">
			...
			<%if(2!=Integer.parseInt(session.getAttribute("userType").toString())){%>
			<span>
			姓名：<input id="studentList" class="easyui-textbox" name="studentList" />
			</span>
			<%}else{%>
			...
			<span>
			班级：<input id="classList" class="easyui-textbox" name="className" />
			</span>
			<%}else{%>
			<span style="display: none">
			班级：<input id="classList" class="easyui-textbox" name="className" />
