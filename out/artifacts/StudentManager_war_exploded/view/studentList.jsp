	...
	$(function() {
	    ...
	    $('#dataList').datagrid({ 
			...
			onLoadSuccess:function () {
				...
				if(3 == <%=Integer.parseInt(session.getAttribute("userType").toString())%>) {
					if (flag) {

					$('#dataList').datagrid('load', {

						clazzid: <%=session.getAttribute("classID")%>
					});
					flag = false
				}
				}
	    ...
	    $("#delete").click(function(){
	  	...
	  	$("#clazzList").combobox({
            ...
            loadFilter:function(data){
				var class_ID=0;
				try{
					class_ID=<%=session.getAttribute("classID")%>
				}catch (e) {
					e.printStackTrace()
				}
	  ...
	  //搜索按钮监听事件
        ...
        $("#clear-btn").click(function(){
            ...
            $("#search_student_name").combobox('clear');
        });
	});
	//上传图片按钮事件
...
<body>
	...
	<div id="toolbar">
		...
		<div style="float: left;margin-top:4px;border-left: none" class="datagrid-btn-separator" >&nbsp;&nbsp;姓名：<input id="search_student_name" class="easyui-textbox" name="search_student_name" /></div>
		<%}else{%>
		...
		<div style="float: left;margin-left: 10px;margin-top:4px;" >
			<%if(1==Integer.parseInt(session.getAttribute("userType").toString())){%>
			<span>
	...
	<div id="addDialog" style="padding: 10px">  
		<div style="float: right; margin: 20px 20px 0 0; width: 200px; border: 0px solid #EBF3FF" id="photo">
	    	<%--<img alt="照片" style="max-width: 200px; max-height: 400px;" title="照片" src="PhotoServlet?method=getPhoto" />--%>
				...
				<img id="add_photo" alt="照片" style="max-width: 200px; max-height: 400px;" title="照片" src="../file/logo.jpg" />
				<form id="uploadForm" method="post" enctype="multipart/form-data" action="PhotoServlet?method=SetPhoto" target="photo_target">
					...
					<input onclick="uploadPhoto()" class="easyui-linkbutton" style="width: 50px; height: 24px;" type="button" value="上传"/>
				</form>
	    </div>
    	...
    	<form id="editForm" method="post">
	    	...
	    	<table cellpadding="8" >
	    		...
	    		<tr>
	    			<td>性别:</td>
	    			<td>
							<select id="edit_sex" class="easyui-combobox" data-options="editable: false, panelHeight: 50, width: 60, height: 30" name="sex"><option value="男">男</option><option value="女">女</option></select>
				...
				<tr>
					<td>密码:</td>
					<td>
						<input id="edit_password" style="width: 200px; height: 30px;" class="easyui-textbox" type="password" name="password" />
					</td>
				</tr>
	    		<tr>
	    			<td>电话:</td>
	    			<td>
							<input id="edit_mobile" style="width: 200px; height: 30px;" class="easyui-textbox" type="text" name="mobile" validType="mobile" />
