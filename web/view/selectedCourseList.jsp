...
<head>
	...
	$(function() {
	    ...
	    $('#dataList').datagrid({ 
	        ...
	        columns: [[  
                ...
                {field:'status',title:'状态',width:200,
                    formatter: function(value,row,index){
                        if (row.status){
                            var statusList = $("#statusList").combobox("getData");
                            for(var i=0;i<statusList.length;i++ ){
                                //console.log(clazzList[i]);
                                if(row.status == statusList[i].id)return statusList[i].name;
                            }
                            return row.status;
                        } else {
                            return 'not found';
	  ...
	  //下拉框通用属性
	  	$("#add_studentList, #add_courseList,#studentList,#courseList,#statusList").combobox({
	  		width: "200",
	  		height: "30",
	  		valueField: "id",
	  		textField: "name",
	  ...
	  //添加信息课程选择框
	    ...
	    //搜索按钮监听事件
	  	$("#search-btn").click(function(){
	  		$('#dataList').datagrid('load',{
                ...
                status: $("#statusList").combobox('getValue') == '' ? 0 : $("#statusList").combobox('getValue')
	  		});
	  	});
        ...
        $("#check").click(function(){
            ...
            if(selectRows.length != 1){
                ...
                if(selectRows[0].status != "待审核"){
                    $.messager.alert("消息提醒", "该选课信息已被审核，请勿重复审核!", "warning");
                    return;
                }
            ...
            buttons: [
                {
                    ...
                    handler:function(){
                        ...
                        if(!validate){
                            ...
                            console.log(status)
                            var data = {id:id, studentid:studentid, status:status};

                            $.ajax({
                                type: "post",
                                url: "SelectedCourseServlet?method=UpdateSelectedCourse",
                                data: data,
                                success: function(msg){
                                    if(msg == "success"){
...
<body>
	...
	<div id="toolbar">
		...
		<div style="margin-top: 3px;">
			...
			状态：<input id="statusList" class="easyui-combobox" data-options="
			valueField:'id',
			textField:'name',
			data:[{
			id:'待审核',
